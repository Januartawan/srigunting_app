...
String _formatErrorData(dynamic data){
  ...
  if(data is Map){
    ...
    data.forEach((key,value){
      ...
      }else if(value is String){errorMessages.add('$key: $value');}
      else{errorMessages.add('$key: ${value.toString()}');}
    });
    return errorMessages.join('\n');
  }
  return data.toString();
}
void yieldGlobalLoading(){
  if(restTask.isEmpty){loadingStreamController.sink.add(false);}
  else{loadingStreamController.sink.add(true);}
}
...
Future<Result<T>> safeCallApi<T>(Future<Response<dynamic>> call,ResponseConverter<T> converter) async{
  ...
  try{
    ...
    restTask.remove(requestID);
    yieldGlobalLoading();
    return Result.error(e.type,e.response?.statusCode??500,e.response?.data??"Network error");
  }catch(e,stackTrace){
    logError("dio error",e);
      ...
      if(result.errorBody!=null){
        if(result.errorBody is Map){
          ...
          if(message=='Unauthenticated.'){
            logError("responseHandler","Unauthenticated response detected, performing auto logout");
            KiwiContainer().resolve<LocalStorageRepository>("secure_storage").deleteAll().then((value){NavigationService.instance.navigateTo(Routing.LOGIN);});
            errorMessage='Sesi login anda berakhir!';
          }else{
            if(message.isNotEmpty && dataError.isNotEmpty){errorMessage='$message\n$dataError';}
            else if(message.isNotEmpty){errorMessage=message;}
            else if(dataError.isNotEmpty){errorMessage=dataError;}
            else{errorMessage='Terjadi kesalahan';}
          }
        }else{errorMessage=result.errorBody.toString();}
      }
      logError("responseHandler error","Status: ${result.status}, Code: ${result.code}, ErrorBody: $errorMessage");
